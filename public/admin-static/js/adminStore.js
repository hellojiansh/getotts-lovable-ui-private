...
const AdminStore = {
    ...
    addInventoryItem(item) {
        ...
        const skipBackendSync = item.__skipBackendSync === true;
        delete item.__skipBackendSync;
        ...
        if (!skipBackendSync) {
            fetch(`${API_BASE}/admin/inventory`, {
                ...
                body: JSON.stringify({
                    ...
                    auth_data: item.auth_data || undefined,
    ...
    addInventoryBulk(items) {
        const inv = this._get(STORE_KEYS.inventory);
        const skipBackendSync = items.every(item => item.__skipBackendSync === true);
        items.forEach(item => {
            delete item.__skipBackendSync;
            item.id = item.id || this.uuid();
            item.status = item.status || 'available';
        ...
        if (!skipBackendSync && items.length > 0) {
            fetch(`${API_BASE}/admin/inventory/bulk-add`, {
                ...
                body: JSON.stringify({
                    ...
                    accounts: items.map(i => ({ id: i.id, email: i.email, password: i.password, auth_data: i.auth_data || undefined, notes: i.notes || undefined }))
                })
            }).catch(console.warn);
        }
        ...
        for (const [id, ov] of Object.entries(overrides)) {
            if (!allKnownIds.has(id) && !allKnownSlugs.has(id) && !(ov.planTypes || ov.prices) && !ov._deleted) {
                mergedProducts.push({
                    ...
                    isHot: ov.isHot || false,
                    featured_position: ov.featured_position || ov.featuredPosition || 0,
                    featuredPosition: ov.featuredPosition || ov.featured_position || 0,
                    ...
                    usage_type: ov.usage_type || 'auto',
                    usage_steps: Array.isArray(ov.usage_steps) ? ov.usage_steps : [],
                    usage_note: ov.usage_note || '',
    ...
    bootstrapDefaults() {
        ...
        let changed = false;
        const productionPayGateUrl = (window.GETOTTS_CONFIG && window.GETOTTS_CONFIG.PAYGATE_FALLBACK) || 'https://paygate.getotts.com';
        if (!s.paygate_url || s.paygate_url.includes('localhost') || s.paygate_url !== productionPayGateUrl) {
            s.paygate_url = productionPayGateUrl;
            changed = true;
        }
